...
IGNORED = [
    "INFO Ignoring batchCount (4) from config_file batch.json: not used in i2v jobs; the job's run_count sets the number of runs",
    "INFO Ignoring width (640) from config_file batch.json: desired_input_width/desired_input_height set the size (832x448)",
    "INFO Ignoring height (448) from config_file batch.json: desired_input_width/desired_input_height set the size (832x448)",
...
class JobOutputTests(JobTestCase):
    def setUp(self) -> None:
        ...
        self.write_base_config({**BASE_CONFIG, "batchCount": 4, "width": 640}, name="batch.json")
        ...
        self.job_path = self.write_job(job_data(run_count=3, prompt_pairs=pairs, input="photo.jpg", desired_input_width=850, config_file="batch.json", config_override={"steps": 8}))
    ...
    def test_validate_job_output_with_a_random_seed(self) -> None:
        ...
        job_path = self.write_job(job_data(run_count=1, prompt_pairs=[{"name": "only", "positive": "text"}], config_file="noseed.json", cooldown_seconds=0), name="noseed.yaml")
        output = self.invoke("validate-job", str(job_path))
        self.assertEqual(output.splitlines()[-6:], ["  cooldown: none (job)", "  input: ROOT/input/first-frame.png", "  output directory: ROOT/output/sunset-walk", "  config file: noseed.json", "  model: m.ckpt", "  seed: (random, drawn when the job starts) (random)"])
        self.assertEqual(self.logged, [])

    def test_dry_run_output(self) -> None:
        output = "ROOT/output/sunset-walk/sunset-walk-20260924-153012"
        expected = "# Job sunset-walk (i2v): 3 runs, seed 42 (config_file), cooldown 90 s (global_config)\n" "# Output names are examples; a real run generates new ones.\n" "# Run 1/3 (pair walk)\n" f"draw-things-cli generate --model base.ckpt --prompt walk --negative-prompt blurry --steps 8 --width 832 --height 448 --seed 42 --config-json '{CONFIG_JSON}' --image '<photo.jpg resized to 832x448>' --output {output}-1000.mov\n" "# Cooldown 90 s\n" "# Run 2/3 (pair wave)\n" f"draw-things-cli generate --model base.ckpt --prompt wave --steps 8 --width 832 --height 448 --seed 42 --config-json '{CONFIG_JSON}' --image {output}-1000-last-frame.png --output {output}-1001.mov\n" "# Cooldown 90 s\n" "# Run 3/3 (pair walk)\n" f"draw-things-cli generate --model base.ckpt --prompt walk --negative-prompt blurry --steps 8 --width 832 --height 448 --seed 42 --config-json '{CONFIG_JSON}' --image {output}-1001-last-frame.png --output {output}-1002.mov\n"
    ...
    def test_job_summary_can_describe_a_random_seed_its_own_way(self) -> None:
        self.write_base_config({"model": "m.ckpt", "width": 832, "height": 448}, name="noseed.json")
        random_job = load_job(self.write_job(job_data(config_file="noseed.json"), name="noseed.yaml"), self.global_config)

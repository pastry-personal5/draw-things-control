...
application through the locally installed `draw-things-cli`. Phase 1 (CLI and jobs) is done; Phase 2 (TUI, state store, run lock) is in progress (Milestones 01 to 07 done); Phase 3 (API and MCP servers) is planned. See
